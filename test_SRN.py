...
parser.add_argument("--logdir", type=str, default="logs/dataset/SRN/", help='path to model and log files')
parser.add_argument("--data_path", type=str, default="dataset/...", help='path to testing data')
parser.add_argument("--save_path", type=str, default="results/dataset/SRN", help='path to save results')
...
def main():
    ...
    # Build model
    print('Loading model ...\n')
    model = SRN(opt.recurrent_iter, opt.use_GPU)
    print_network(model)
    if opt.use_GPU:
        model = model.cuda()
